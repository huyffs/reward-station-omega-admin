...
export async function canSudo(jwt: JsonWebToken): Promise<boolean> {
	return (await isValidJwt(jwt)) && !!jwt.payload.admin;
}

export async function hasOrgClaim(jwt: JsonWebToken, orgId: string): Promise<boolean> {
	return (await isValidJwt(jwt)) && jwt.payload.orgs[orgId] > 0;
}
