<script lang="ts">
	...
	const engagements = derived(engagementStore.store, ($engagementStore) => {
		return Object.values($engagementStore)
			.sort(sortByUpdatedAt)
			.filter((engage) => {
				return engage.campaign_id === $page.params.campaignId && engage.chain_id === chainId;
			});
	});
...
<main class="max-w-screen-md my-8 mx-auto">
	<div class="flex gap-8 items-center">
		<h1 class="h1">
			Engagements on {CHAINS.find((c) => c.id === chainId)?.name || 'unknown chain'}
		</h1>
	</div>
